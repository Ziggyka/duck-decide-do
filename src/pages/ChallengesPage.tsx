...
import {
  ...
  ChevronRight, Check, Flame, HelpCircle, Calendar, Target,
  Shield, Crown, Gem, Bookmark, X, PartyPopper
} from "lucide-react";
...
const mainChallenge = {
  ...
  description: "Saia da rotina gastronômica! Encontre um restaurante que ninguém do grupo conhece e compartilhem a experiência. Tire fotos, avaliem juntos e adicionem à lista do grupo.",
...
const weeklyChallenges = [
  { id: 1, title: "Jogue um jogo cooperativo com amigos", icon: "🎮", desc: "Escolha um jogo co-op e jogue pelo menos 1 hora", type: "Grupo", xp: 150, item: "Carta Dourada", status: "available" },
  { id: 2, title: "Conheça um lugar novo na sua cidade", icon: "📍", desc: "Explore um bairro ou ponto que nunca visitou", type: "Solo", xp: 120, item: "Mapa Secreto", status: "completed" },
  { id: 3, title: "Veja um anime curto de comédia", icon: "🍿", desc: "Assista algo fora do seu gênero habitual", type: "Solo", xp: 100, item: "Ticket VIP", status: "available" },
  { id: 4, title: "Monte uma playlist para um rolê", icon: "🎵", desc: "Crie uma playlist temática e compartilhe com amigos", type: "Grupo", xp: 80, item: "Microfone Dourado", status: "available" },
  { id: 5, title: "Organize uma noite de jogos de tabuleiro", icon: "🎲", desc: "Reúna a galera para jogar presencialmente", type: "Grupo", xp: 200, item: "Dado Mágico", status: "available" },
  { id: 6, title: "Descubra uma receita nova e cozinhe", icon: "🍳", desc: "Faça algo que nunca cozinhou antes", type: "Solo", xp: 100, item: "Chapéu de Chef", status: "available" },
];
...
const groupChallenges = [
  { group: "Turma do Cinema", challenge: "Assistam 3 filmes juntos essa semana", participants: [duckAvatar1, duckAvatar2, duckAvatar3], progress: 66 },
  { group: "Galera do Board Game", challenge: "Testem um jogo novo que ninguém jogou", participants: [duckAvatar2, duckAvatar3], progress: 0 },
];
...
const ChallengesPage = () => {
  ...
  const completedCount = weeklyChallenges.filter(c => c.status === "completed").length + acceptedIds.length;

  const handleAcceptMain = () => {
    setAcceptedMain(true);
    toast.success("Desafio aceito! Boa sorte, pato! 🦆🔥");
  };

  const handleAcceptWeekly = (id: number) => {
    setAcceptedIds(prev => [...prev, id]);
    toast.success("Desafio aceito! Vai pra cima! 🚀");
  };
      ...
      <div className="max-w-5xl mx-auto space-y-8 pb-12">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button className="gap-2 pato-btn-bounce">
              <Sparkles className="w-4 h-4" /> Gerar novos desafios
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="pato-card flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Tendência entre amigos</p>
              <p className="text-sm font-semibold">Noites de cinema em alta!</p>
            </div>
          </div>
        </div>

        {/* Section 1 — Main Challenge */}
        <section>
          ...
          <div className="pato-card p-6 border-primary/30 relative overflow-hidden">
            ...
            <div className="flex gap-6 relative z-10">
              <div className="w-24 h-24 rounded-2xl bg-accent/15 flex items-center justify-center text-5xl flex-shrink-0">
                🍽️
              </div>
              <div className="flex-1 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <Badge variant="outline" className="mb-2 text-primary border-primary/30 text-[10px]">
                ...
                {showReason && (
                  <p className="text-xs text-muted-foreground bg-muted/50 rounded-lg px-3 py-2 animate-fade-in">
                    💡 {mainChallenge.reason}
                  </p>
                )}
                ...
                <div className="flex gap-2 pt-1">
                  {acceptedMain ? (
                    <Button disabled className="gap-2 bg-success text-success-foreground">
                      <Check className="w-4 h-4" /> Desafio Aceito!
                    </Button>
                  ) : (
                    <Button onClick={handleAcceptMain} className="gap-2 pato-btn-bounce">
                      <Target className="w-4 h-4" /> Aceitar desafio
                    </Button>
                  )}
                  <Button variant="outline" size="sm" className="gap-1" onClick={() => toast("Desafio ignorado")}>
                    <X className="w-3.5 h-3.5" /> Ignorar
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
                    className={`gap-1 ${savedMain ? "text-accent" : ""}`}
                    onClick={() => { setSavedMain(!savedMain); toast(savedMain ? "Removido dos salvos" : "Salvo para depois! 🔖"); }}
                  >
        ...
        {/* Section 2 — Weekly Challenges Grid */}
        <section>
          <h2 className="text-lg font-display font-bold mb-4 flex items-center gap-2">
          ...
          <div className="grid grid-cols-3 gap-4">
            {weeklyChallenges.map((c) => {
              ...
              return (
                <div key={c.id} className={`pato-card p-5 flex flex-col gap-3 transition-all ${isCompleted ? "opacity-70" : ""} ${isAccepted ? "border-primary/40 glow-duck" : ""}`}>
                  <div className="flex items-start justify-between">
                    <span className="text-3xl">{c.icon}</span>
                    {isCompleted && <Badge className="bg-success/15 text-success border-0 text-[10px]"><Check className="w-3 h-3 mr-1" /> Feito</Badge>}
                    {isAccepted && !isCompleted && <Badge className="bg-primary/15 text-primary border-0 text-[10px]">Aceito</Badge>}
                  </div>
                  ...
                  {!isCompleted && !isAccepted && (
                    <Button size="sm" className="w-full pato-btn-bounce mt-1" onClick={() => handleAcceptWeekly(c.id)}>
                      Aceitar
                    </Button>
                  )}
                </div>
        ...
        <section>
          <h2 className="text-lg font-display font-bold mb-4 flex items-center gap-2">
            <Target className="w-5 h-5 text-accent" /> Seu Progresso
          </h2>
          <div className="pato-card p-6">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-medium">Você completou <span className="font-bold text-primary">{completedCount}</span> de <span className="font-bold">6</span> desafios</p>
              <div className="flex items-center gap-2 text-sm font-semibold">
                <Flame className="w-4 h-4 text-destructive" />
                <span>Streak: 3 semanas 🔥</span>
              </div>
            </div>
            <Progress value={(completedCount / 6) * 100} className="h-4" />
            <div className="flex gap-2 mt-4">
              {weeklyChallenges.map((c) => {
                const done = c.status === "completed" || acceptedIds.includes(c.id);
                return (
                  <div key={c.id} className={`flex-1 h-2 rounded-full transition-all ${done ? "bg-primary" : "bg-muted"}`} />
                );
              })}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {groupChallenges.map((g) => (
              <div key={g.group} className="pato-card p-5 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">{g.progress}% completo</span>
          ...
          <div className="pato-card divide-y divide-border">
            {history.map((h, i) => (
              <div key={i} className="flex items-center justify-between px-5 py-3.5">
                ...
                <div className="flex items-center gap-3">
                  <Badge variant="outline" className={`text-[10px] ${h.status === "Completo" ? "text-success border-success/30" : "text-destructive border-destructive/30"}`}>
                    {h.status}
                  </Badge>
